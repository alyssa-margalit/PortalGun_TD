...
def on_connect(client, userdata, flags, rc):
    ...
    """
    client.subscribe("alyssasrpi/defaultCallback")
...
if __name__ == '__main__':
    ...
    while (True):
        if select == 1:
            #publish a float
            client.publish("alyssasrpi/defaultCallback", 1.23456)
            select ^= 1
        elif select == 0:
            #publish a string
            client.publish("alyssasrpi/customCallback", "hello this is a string")
            client.publish("tdCallback",'message from computer')
            select ^= 1
        time.sleep(1)